    ...
    try {
      ...
      if (resp?.statusText === "Created") {
        if (resp?.data?.errors?.length > 0) {
          toast.error(
            <div>
              {resp?.data?.errors?.map((errorObj, index) => (
                <div key={index}>{errorObj?.error}</div>
              ))}
            </div>
          );
        }
        ...
        if (invalidTags) {
          dispatch(apiSlice.util.invalidateTags(invalidTags));
        }
      ...
      toast.error(
        <div>
          {err?.response?.data?.errors?.map((errorObj, index) => (
            <div key={index}>{errorObj?.error}</div>
          ))}
        </div>
      );
...
export default LeadsCreateMany;
