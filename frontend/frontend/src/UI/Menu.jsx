    ...
    return subMenu?.map((item) => {
      ...
      if (
        ...
        content = (
          <li
            key={item?.key}
            className="menu-item pl-8 py-2 my-1 font-Popins rounded-2xl text-gray-600">
        ...
        content = (
          <li
            key={item?.key}
            className="menu-item pl-8 py-2 my-1 font-Popins text-gray-600">
            <div className="flex items-center gap-1 w-full">
              {/* {item?.icon} */}
              <span className="w-full">{item?.label}</span>
            </div>
          ...
          itemContent = (
            <li
              ...
              className={cn(
                ...
                {
                  "menu-item px-4 rounded-2xl": !item?.children,
                },
                {
                  "menu-item-with-children rounded-2xl": item?.children,
                },
                {
                  "flex items-center rounded-none ": collapsed,
                }
              )}>
              <div
                ...
                {collapsed ? (
                  ...
                  <>
                    <div
                      className={cn(
                        "flex items-center gap-3 w-full text-[15px] text-gray-600"
                      )}>
              ...
              <div
                ...
                {collapsed ? (
                  ...
                  <>
                    <div
                      className={cn(
                        "flex items-center gap-3 w-full text-gray-600"
                      )}>
                      <span className="text-[15px]">{item?.icon}</span>
                      <span className="w-full text-[15px] mt-1">
                        {item?.label}
                      </span>
                    </div>
                    {item?.children && (
