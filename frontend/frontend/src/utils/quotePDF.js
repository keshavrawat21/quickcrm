...
export default function quotePDF(data, companyInfo, title, currencyName) {
  ...
  const bottomRightContent = [
    {
      label: "Total Amount:",
      value: `${data?.totalAmount ? Number(data.totalAmount).toFixed(2) : 0}`,
    },
    {
      label: "Total discount:",
      value: `${data?.discount ? Number(data.discount).toFixed(2) : 0}`,
    },
  ];
  const tableHead = [
    "SL",
  ...
  const tableBody = data.quoteProduct.map((item, index) => [
    index + 1,
    item?.product?.name,
  ...
  const TopRightInfo = [
    ...
    {
      value: data?.quoteStage?.quoteStageName
        ? `Quote Stage: ${data?.quoteStage?.quoteStageName}`
        : "",
    },
    {
      value: `Expiration Date: ${moment(data?.data?.expirationDate).format(
        "ll"
      )}`,
    },
  ];
