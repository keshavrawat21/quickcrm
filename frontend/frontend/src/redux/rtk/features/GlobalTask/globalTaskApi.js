  ...
  endpoints: (builder) => ({
    ...
    addProjectTask: builder.mutation({
      ...
      async onQueryStarted(arg, { queryFulfilled, dispatch }) {
        try {
          ...
          toastHandler(err.error.data.error, "warning");
        }
      },
      invalidatesTags: ["Tasks", "TaskStatusById"],
    }),
    ...
    deleteProjectTask: builder.mutation({
      ...
      async onQueryStarted(arg, { queryFulfilled, dispatch }) {
        ...
        try {
          await queryFulfilled;
          toastHandler("Deleted ProjectTask successful", "success");
        } catch (err) {
          toastHandler("Something went wrong, Please try again", "warning");
        }
      },
      invalidatesTags: ["Task", "Tasks", "TaskStatusById"],
    }),
  }),
});
