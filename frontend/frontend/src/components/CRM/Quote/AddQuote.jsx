...
export default function AddQuote({ createAs, drawer }) {
  ...
  const totalCalculator = () => {
    ...
    const subTotal =
      productArray?.reduce((subTotal, current) => {
        ...
        const discount = current?.productDiscount || 0;

        return [
          ...subTotal,
          {
            subVat: current?.productVat || 0,
            subPrice: price * quantity + totalVat - discount,
          },
        ];
      }, []) || [];
        ...
        <div className="-mt-5 flex justify-between gap-8">
          ...
          <Form.Item label="Quote Owner" name="quoteOwnerId" className="w-full">
            <Select
              ...
              loading={!allStaff}
              showSearch
              placeholder="Select a Quote Owner "
